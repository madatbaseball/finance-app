...
# ── 세션 초기화 (로그인 없이 게스트로 바로 진입) ─────────────────
if "page" not in st.session_state:
    st.session_state.page = "홈"
if "portfolio" not in st.session_state:
    st.session_state.portfolio = []
if "authenticated" not in st.session_state:
    st.session_state.authenticated = True  # 항상 로그인 상태
if "user" not in st.session_state:
    st.session_state.user = {"id": "guest", "username": "게스트", "email": ""}
if "auth_mode" not in st.session_state:
    st.session_state.auth_mode = "login"
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = False

# ── DB 함수 (게스트 모드에서는 로컬 세션만 사용) ─────────────────
...
st.markdown("""
    ...
    .metric-change-up   { font-size: 13px; font-weight: 600; color: #f04452; margin-top: 6px; }
    .metric-change-down { font-size: 13px; font-weight: 600; color: #1b64da; margin-top: 6px; }
    .metric-change-flat { font-size: 13px; font-weight: 600; color: #8b95a1; margin-top: 6px; }

    h1 { font-size: 22px !important; font-weight: 800 !important; color: #191f28 !important; letter-spacing: -0.5px !important; margin-bottom: 4px !important; }
    ...
    details[data-testid="stExpander"] summary:hover { background: #f8f9fa !important; }
    ...
    div[data-testid="metric-container"] label {
        ...
        color: #8b95a1 !important; letter-spacing: 0.2px !important;
    }
    div[data-testid="metric-container"] [data-testid="metric-value"] {
        font-size: 22px !important; font-weight: 800 !important;
        color: #191f28 !important; letter-spacing: -0.5px !important;
    }

    .stTabs [data-baseweb="tab-list"] {
        gap: 4px; background: #f2f4f6; border-radius: 10px; padding: 4px;
    }
    .stTabs [data-baseweb="tab"] {
        border-radius: 8px; font-size: 13px; font-weight: 600;
        color: #8b95a1; padding: 8px 16px; border: none !important;
    }
    ...
    .stTextInput > div > div > input {
        border: 1.5px solid #e5e8eb; border-radius: 10px; background: #ffffff;
        font-size: 14px; padding: 10px 14px; color: #191f28; transition: border-color 0.15s;
    }
    .stTextInput > div > div > input:focus {
        border-color: #3182f6; box-shadow: 0 0 0 3px rgba(49,130,246,0.12);
    }
    .stSelectbox > div > div { border: 1.5px solid #e5e8eb; border-radius: 10px; background: #ffffff; font-size: 14px; }
    .stNumberInput > div > div > input { border: 1.5px solid #e5e8eb; border-radius: 10px; font-size: 14px; }

    .stDataFrame { border-radius: 12px; border: none !important; box-shadow: 0 2px 8px rgba(0,0,0,0.05); overflow: hidden; }

    .stRadio > div { gap: 8px; }
    .stRadio label { font-size: 13px !important; font-weight: 500 !important; }

    div[data-testid="stForm"] button[kind="primaryFormSubmit"],
    div[data-testid="stForm"] button {
        background: #3182f6 !important; color: white !important; border: none !important;
        border-radius: 10px !important; font-size: 14px !important; font-weight: 600 !important;
        padding: 10px 20px !important; transition: background 0.15s, transform 0.1s !important;
    }
    div[data-testid="stForm"] button:hover { background: #1b64da !important; transform: translateY(-1px) !important; }

    .stSpinner > div { border-top-color: #3182f6 !important; }
    [data-testid="stSpinner"] p, [data-testid="stSpinner"] span { display: none !important; }

    .stCaption, [data-testid="stCaptionContainer"] p { font-size: 12px !important; color: #8b95a1 !important; }
...
if st.session_state.dark_mode:
    ...
    .stApp, .stApp *, .block-container * { color: #e6edf3 !important; }
    ...
    div[data-testid="metric-container"] { background: #161b22 !important; border: 1px solid #30363d !important; }
    div[data-testid="metric-container"] label,
    div[data-testid="metric-container"] [data-testid="stMetricLabel"] p { color: #8b949e !important; }
    div[data-testid="metric-container"] [data-testid="metric-value"],
    div[data-testid="metric-container"] [data-testid="stMetricValue"] { color: #f0f6fc !important; }
    .stTextInput > div > div > input,
    .stNumberInput > div > div > input,
    .stTextArea > div > div > textarea {
        background: #21262d !important; border: 1.5px solid #30363d !important; color: #e6edf3 !important;
    }
    .stTextInput > div > div > input::placeholder,
    .stNumberInput > div > div > input::placeholder { color: #6e7681 !important; }
    .stTextInput > div > div > input:focus,
    .stNumberInput > div > div > input:focus { border-color: #58a6ff !important; box-shadow: 0 0 0 3px rgba(88,166,255,0.15) !important; }
    .stSelectbox > div > div, .stSelectbox > div > div > div { background: #21262d !important; border: 1.5px solid #30363d !important; color: #e6edf3 !important; }
    .stSelectbox svg { fill: #8b949e !important; }
    ul[data-testid="stSelectboxVirtualDropdown"], li[role="option"] { background: #21262d !important; color: #e6edf3 !important; }
    li[role="option"]:hover { background: #30363d !important; }
    details[data-testid="stExpander"] { background: #161b22 !important; border: 1px solid #30363d !important; box-shadow: none !important; }
    details[data-testid="stExpander"] summary,
    details[data-testid="stExpander"] summary p,
    details[data-testid="stExpander"] summary span { color: #e6edf3 !important; background: #161b22 !important; }
    details[data-testid="stExpander"] summary:hover { background: #21262d !important; }
    ...
    div.stButton > button { color: #e6edf3 !important; border: 1px solid #30363d !important; background: #21262d !important; }
    div.stButton > button:hover { background: #30363d !important; color: #f0f6fc !important; }
    div[data-testid="stForm"] button { background: #1f6feb !important; color: #ffffff !important; border: none !important; }
    div[data-testid="stForm"] button:hover { background: #388bfd !important; }
    .stCheckbox label, .stRadio label { color: #e6edf3 !important; }
    .stDataFrame, .stDataFrame * { background: #161b22 !important; color: #e6edf3 !important; border-color: #30363d !important; }
    hr { border-color: #30363d !important; }
    .ticker-wrapper { background: #161b22 !important; border-color: #30363d !important; }
    ...
    div[style*="background:#fff"], div[style*="background: #fff"] { background: #161b22 !important; border-color: #30363d !important; color: #e6edf3 !important; }
    ...
    div[style*="color:#444"], div[style*="color: #444"] { color: #e6edf3 !important; }
    div[style*="color:#888"], div[style*="color: #888"],
    div[style*="color:#666"], div[style*="color: #666"] { color: #8b949e !important; }
    ...
    div[style*="background:#f8f9fa"], div[style*="background:#f0f7ff"] { background: #0d1117 !important; }
    </style>
    """, unsafe_allow_html=True)
...
with cols[0]:
    if st.button("홈"):
        st.session_state.page = "홈"; st.rerun()
with cols[1]:
    if st.button("기업 분석"):
        st.session_state.page = "기업 분석"; st.rerun()
with cols[2]:
    if st.button("뉴스"):
        st.session_state.page = "뉴스"; st.rerun()
with cols[3]:
    if st.button("국제 금융"):
        st.session_state.page = "국제 금융"; st.rerun()
with cols[4]:
    if st.button("실시간 주가"):
        st.session_state.page = "실시간 주가"; st.rerun()
with cols[5]:
    if st.button("포트폴리오"):
        st.session_state.page = "포트폴리오"; st.rerun()
with cols[6]:
    if st.button("환율"):
        st.session_state.page = "환율"; st.rerun()
with cols[7]:
    if st.button("경제 캘린더"):
        st.session_state.page = "경제 캘린더"; st.rerun()
with cols[8]:
    if st.button("AI 추천"):
        st.session_state.page = "AI 추천"; st.rerun()
with cols[9]:
    if st.button("서비스 안내"):
        st.session_state.page = "서비스 안내"; st.rerun()
...
def get_stock_data(tickers):
    ...
    def _fetch_krx(sym):
        ...
        try:
            ...
            r = requests.post(
                ...
                data={"bld": "dbms/MDC/STAT/standard/MDCSTAT01501", "mktId": "STK", "trdDd": today,
                    ...
                    if row.get("ISU_SRT_CD") == code:
                        ...
                        if cur > 0:
                            ...
                            return sym, {"종목": None, "현재가": f"{cur:,.0f}원",
                                        "등락": f"{arrow} {abs(chg):,.0f}원", "등락률": f"{pct:+.2f}%"}
        except:
            pass
        return sym, None
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
        for sym, data in ex.map(_fetch_krx, syms):
            if data: result_map[sym] = data
            else: krx_missing.append(sym)
...
def load_stock_list():
    ...
    for corp in root.findall("list"):
        ...
        if stock_code and stock_code.strip():
            stocks.append({"name": name, "ticker": stock_code.strip() + ".KS"})
    return stocks

page = st.session_state.page

if "stock_list_cache" not in st.session_state:
    st.session_state.stock_list_cache = None
...
if page == "홈":
    ...
    .stat-tooltip .tip-icon {
        ...
        cursor:pointer; margin-left:5px; vertical-align:middle; flex-shrink:0;
    }
    ...
    username = user.get("username", "게스트")

    st.title("마이페이지")
    st.markdown(
        f"<div style='font-size:14px;color:#888;margin-bottom:24px;'>"
        f"<b style='color:#1a1e2e;'>데모 버전</b>으로 실행 중입니다. "
        f"회원가입 후 포트폴리오 저장, 목표가 알림 등 모든 기능을 이용하실 수 있습니다.</div>",
        unsafe_allow_html=True
    )

    mp_tab1, mp_tab2 = st.tabs(["화면 설정", "서비스 안내"])
    ...
    div[data-testid="stForm"] button {
        background: #1a2f5e !important; color: white !important; border: none !important;
    ...
    div[data-testid="stForm"] button:hover { background: #2563eb !important; color: white !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    if submitted and company:
        ...
        if not hist_tech.empty:
            ...
            fig_tech.add_trace(go.Candlestick(
                x=idx, open=hist_tech["Open"][-n:], high=hist_tech["High"][-n:],
                low=hist_tech["Low"][-n:], close=close[-n:], name="주가",
            ...
            fig_tech.add_trace(go.Bar(
                ...
                marker_color=vol_colors, opacity=0.6, yaxis="y2", showlegend=False,
            ), row=1, col=1)
            ...
            for lvl, col in [(70,"#ef4444"),(30,"#22c55e")]:
                fig_tech.add_hline(y=lvl, line_dash="dash", line_color=col, line_width=1, row=2, col=1)

            fig_tech.add_trace(go.Scatter(x=idx, y=macd_line[-n:], name="MACD",
                line=dict(color="#0ea5e9", width=1.5)), row=3, col=1)
            fig_tech.add_trace(go.Scatter(x=idx, y=signal_line[-n:], name="Signal",
                line=dict(color="#f97316", width=1.5, dash="dot")), row=3, col=1)
            hist_colors = ["#ef4444" if v >= 0 else "#3b82f6" for v in macd_hist[-n:]]
            fig_tech.add_trace(go.Bar(x=idx, y=macd_hist[-n:], name="Histogram",
            ...
            for row_i in [1, 2, 3]:
                fig_tech.update_yaxes(gridcolor="#f3f4f6", tickfont=dict(size=10, color="#888"),
    ...
    with news_main_tab1:
        ...
        selected_category = st.selectbox("분야 선택", list(NEWS_CATEGORIES.keys()),
                                         key="news_category", label_visibility="collapsed")
        ...
        for item in news_items:
            ...
            for line in lines:
                ...
                if line.startswith("제목:"): title = line.replace("제목:", "").strip(); in_detail = False
                elif line.startswith("출처:"): source = line.replace("출처:", "").strip(); in_detail = False
                elif line.startswith("날짜:"): date_str = line.replace("날짜:", "").strip(); in_detail = False
                elif line.startswith("중요도:"): importance = line.replace("중요도:", "").strip().upper(); in_detail = False
                elif line.startswith("요약:"): summary = line.replace("요약:", "").strip(); in_detail = False
                elif line.startswith("상세:"): detail_lines = [line.replace("상세:", "").strip()]; in_detail = True
                elif in_detail: detail_lines.append(line)
            ...
            with st.expander(expander_label):
                ...
                if is_high:
                    st.markdown(f"<div style='background:#fff0f0;border-left:3px solid #ef4444;padding:8px 12px;border-radius:4px;font-size:13px;color:#b91c1c;margin-bottom:10px;'>⚠️ 주가 영향 주목 — {summary}</div>", unsafe_allow_html=True)
                else:
                    st.markdown(f"<div style='font-size:13px;color:#666;margin-bottom:10px;'>핵심: {summary}</div>", unsafe_allow_html=True)
                if detail: st.markdown(detail)
            count += 1
            if count >= 10: break
...
1. 미국 시장 동향 2. 중동 및 유가 동향 3. 아시아 시장 동향 4. 환율 동향 5. 주요 리스크 요인
규칙: 대괄호 각주 절대 금지. 별표 마크다운 절대 금지. 분석 불가 안내 절대 금지.
""")
        result = re.sub(r'\[\d+\]', '', result)
        result = re.sub(r'\*\*?(.*?)\*\*?', r'\1', result)
        return result
    ...
    with tab3:
        st.caption("미국 나스닥·NYSE 등 해외 주식 티커를 직접 입력하세요. 예: AAPL, TSLA")
    ...
    with tab2:
        ...
        if search_q:
            ...
            if matched:
                matched_with_code = [f"{n}  ({stock_map.get(n, '').replace('.KS', '')})" for n in matched]
                ...
                if selected_ticker:
                    ...
                    period_map = {
                        "1일": ("1d", "1m"), "3일": ("5d", "5m"), "1년": ("1y", "1d"),
                        "3년": ("3y", "1wk"), "10년": ("10y", "1mo"),
                    }
                    ...
                    with st.spinner(f"{selected_name} 주가 불러오는 중..."):
                        try:
                            ...
                            for suffix in [".KS", ".KQ"]:
                                ...
                                if not h.empty:
                                    hist = h; selected_ticker = code + suffix; break
                            ...
                            if not hist.empty:
                                ...
                                if selected_period in ["1일", "3일"]:
                                    tickformat = "%m/%d<br>%H:%M"; dtick = 3600000 * 2 if selected_period == "1일" else 3600000 * 6
                                elif selected_period == "1년":
                                    tickformat = "%Y/%m"; dtick = "M1"
                                elif selected_period == "3년":
                                    tickformat = "%Y/%m"; dtick = "M3"
                                else:
                                    tickformat = "%Y"; dtick = "M12"

                                fig = go.Figure()
                                fig.add_trace(go.Scatter(x=idx, y=hist["Close"], mode="lines",
                                    line=dict(color="#2563eb", width=2),
                                    hovertemplate="%{x|%Y/%m/%d %H:%M}<br>%{y:,.0f}원<extra></extra>"))
                                fig.update_layout(
                                    height=400, margin=dict(l=0, r=0, t=20, b=0),
                                    paper_bgcolor="white", plot_bgcolor="white",
                                    xaxis=dict(tickformat=tickformat, dtick=dtick, tickangle=0,
                                               showgrid=True, gridcolor="#f0f0f0", tickfont=dict(size=11)),
                                    yaxis=dict(showgrid=True, gridcolor="#f0f0f0", tickformat=",", tickfont=dict(size=11)),
                                    showlegend=False,
                                )
    ...
    with tab4:
        ...
        st.caption("외국인·기관 순매수/순매도 동향을 AI가 분석합니다.")

        sd_col1, sd_col2 = st.columns([2, 1])
        with sd_col1:
            sd_search = st.text_input("종목명 검색 (비워두면 시장 전체)", placeholder="예: 삼성전자", key="sd_search")
        with sd_col2:
    ...
    if _time.time() - _last_alert_check > 300:
    ...
    with pf_main_tab1:
        ...
        buy_price = st.number_input("매수 평균가 (원)", min_value=0, value=None,
                                    placeholder="예: 73000", step=100, format="%d", key=f"pf_price_{ver}")

        if st.button("종목 추가", key="add_btn"):
            if selected_stock and buy_price and buy_price > 0:
                ticker_auto = stock_map.get(selected_stock, "")
                new_item = {"name": selected_stock, "ticker": ticker_auto,
                            "qty": qty_input, "buy_price": buy_price, "id": "guest_local"}
        ...
        if st.session_state.portfolio:
            ...
            def _get_pf_prices(tickers_tuple):
                ...
                try:
                    raw = yf.download(tickers_list, period="5d", interval="1d",
                                      group_by="ticker", auto_adjust=True, progress=False, threads=True, timeout=15)
                    prices = {}
                    for sym in tickers_list:
                        try:
                            if len(tickers_list) == 1: h = raw
                            else: h = raw[sym] if sym in raw.columns.get_level_values(0) else pd.DataFrame()
                            if h is not None and not h.empty:
                                h = h.dropna(subset=["Close"])
                                if not h.empty: prices[sym] = float(h["Close"].iloc[-1])
                        except: pass
                    return prices
                except: return {}
                ...
                try:
                    ...
                    if h.empty:
                        ...
                        for sfx in [".KS", ".KQ"]:
                            ...
                            if not h2.empty: h = h2; item["ticker"] = code + sfx; break
                    return float(h["Close"].iloc[-1]) if not h.empty else item["buy_price"]
                except: return item["buy_price"]
            ...
            for item, current in zip(pf_items, prices):
                ...
                rows.append({
                    "종목": item["name"], "섹터": sector, "보유수량": item["qty"],
                    "매수가": f"{item['buy_price']:,.0f}원", "현재가": f"{current:,.0f}원",
                    "투자금액": f"{invested:,.0f}원", "평가금액": f"{value:,.0f}원",
                    "수익금액": f"{profit:+,.0f}원", "수익률": f"{profit_pct:+.2f}%",
                })
            ...
            c3.metric("총 수익금액", f"{total_profit:+,.0f}원", f"{total_pct:+.2f}%")
            ...
            pastel_colors = ["#a5b4fc","#86efac","#fde68a","#fca5a5","#67e8f9",
                             "#f9a8d4","#c4b5fd","#6ee7b7","#fcd34d","#93c5fd","#fb923c"]

            fig_sector = go.Figure(go.Pie(
                labels=s_labels, values=s_vals, hole=0.45,
            ...
            fig_sector.update_layout(
                ...
                paper_bgcolor="white", showlegend=True, legend=dict(font=dict(size=11), orientation="v")
            )
            ...
            fig_stock = go.Figure(go.Pie(
                labels=stock_labels, values=stock_vals, hole=0.45,
            ...
            with st.spinner("3개월 수익률 데이터 불러오는 중..."):
                try:
                    ...
                    if all_hist:
                        ...
                        fig_ret = go.Figure()
                        fig_ret.add_trace(go.Scatter(x=port_return.index, y=port_return.values, name="내 포트폴리오",
                            line=dict(color="#6366f1", width=2.5),
                            hovertemplate="%{x|%Y/%m/%d}<br>%{y:+.2f}%<extra>포트폴리오</extra>"))
                        fig_ret.add_trace(go.Scatter(x=kospi_return.index, y=kospi_return.values, name="KOSPI",
            ...
            with st.spinner("MDD 분석 중..."):
                try:
                    if all_hist and not portfolio_series.empty:
                        ...
                        sharpe_like = (port_final / annual_vol) if annual_vol > 0 else 0

                        fig_dd = go.Figure()
                        fig_dd.add_trace(go.Scatter(
                            x=portfolio_series.index, y=drawdown.values,
                            fill="tozeroy", name="낙폭 (Drawdown)",
                            line=dict(color="#f87171", width=1.5), fillcolor="rgba(248,113,113,0.18)",
            ...
            if st.button("AI 리포트 생성", key="ai_report_btn"):
                ...
                total_str = (f"총 투자금액: {total_invested:,.0f}원 / 총 평가금액: {total_value:,.0f}원 / 총 수익률: {total_pct:+.2f}%")
                report_prompt = (
                    f"아래는 내 주식 포트폴리오야. 전문 자산관리사처럼 한국어로 분석해줘.\n\n"
                    f"[포트폴리오]\n{portfolio_summary}\n\n[요약]\n{total_str}\n\n"
                    f"반드시 아래 형식을 정확히 지켜서 작성해줘:\n\n"
                    f"한줄해결책: [지금 당장 해야 할 가장 핵심적인 한 줄 액션]\n\n"
                ...
                for line in report.split("\n"):
                    line = line.strip()
                    if not line: continue
                    if line.startswith("한줄해결책:"): oneliner = line.replace("한줄해결책:", "").strip()
                    elif line.startswith("현황요약:"): summary_text = line.replace("현황요약:", "").strip(); current_key = None
                    elif _re.match(r'^[1-5]\.\s', line):
                        current_key = line; sections[current_key] = []; section_order.append(current_key)
                    elif current_key: sections[current_key].append(line)
                    elif summary_text and not oneliner: summary_text += " " + line
                ...
                if oneliner:
                    html += (f"<div style='background:#f0f4ff;border-radius:8px;padding:16px 20px;margin-bottom:20px;'>"
                             f"<div style='font-size:11px;color:#6366f1;font-weight:700;margin-bottom:6px;letter-spacing:0.5px;'>💡 지금 당장 해야 할 것</div>"
                             f"<div style='font-size:17px;font-weight:700;color:#1a1e2e;line-height:1.6;'>{oneliner}</div></div>")
                if summary_text:
                ...
                for key in section_order:
                    ...
                    if m: num = m.group(1); title = m.group(2)
                    else: num, title = "", key
                    html += (f"<div style='margin-bottom:20px;'>"
                             f"<div style='font-size:16px;font-weight:700;color:#1a1e2e;margin-bottom:8px;'>{num}. {title}</div>"
                             f"<div style='font-size:14px;color:#444;line-height:1.85;'>{content}</div></div>")
                html += "</div>"
        ...
        if st.button("관심종목 추가", key="wl_add_btn"):
            if wl_selected:
                ...
                if wl_selected in already:
                    ...
                    st.session_state.watchlist.append({"name": wl_selected, "ticker": stock_map.get(wl_selected, "")})
        ...
        if st.session_state.watchlist:
            ...
            for i, item in enumerate(st.session_state.watchlist):
                try:
                    ...
                    if not h5.empty:
                        cur = h5["Close"].iloc[-1]; prev = h5["Close"].iloc[-2] if len(h5) >= 2 else cur
                        chg = cur - prev; chg_pct = chg / prev * 100
                        high52 = h52["Close"].max() if not h52.empty else cur
                        from_high = (cur - high52) / high52 * 100
                        arrow = "▲" if chg >= 0 else "▼"; color = "#e03131" if chg >= 0 else "#1971c2"
                        cur_str = f"{cur:,.0f}원"; chg_str = f"{arrow} {abs(chg):,.0f}원"
                        pct_str = f"{chg_pct:+.2f}%"; high_str = f"{from_high:.1f}%"
                    else:
                        cur_str = chg_str = pct_str = high_str = "-"; color = "#888"
                except:
                    cur_str = chg_str = pct_str = high_str = "-"; color = "#888"
    ...
    with pf_main_tab3:
        ...
        with sim_tab1:
            ...
            with sim_col1: sim_action = st.radio("매수/매도", ["매수", "매도"], horizontal=True, key="sim_action")
            with sim_col2: sim_search = st.text_input("종목명", placeholder="예: 삼성전자", key="sim_search")
            with sim_col3: sim_qty = st.number_input("수량 (주)", min_value=1, value=10, key="sim_qty")
            ...
            if st.button("시뮬레이션 실행", key="sim_run"):
                if not sim_selected:
                    ...
                    if sim_price == 0:
                        ...
                        cur_total = 0; cur_items = []
                        for p in st.session_state.portfolio:
                            try:
                                ph = yf.Ticker(p["ticker"]).history(period="2d")
                                pc = ph["Close"].iloc[-1] if not ph.empty else p["buy_price"]
                            except: pc = p["buy_price"]
                            val = pc * p["qty"]; cur_total += val
                            cur_items.append({"name": p["name"], "value": val})

                        sim_value = sim_price * sim_qty
                        if sim_action == "매수":
                            new_total = cur_total + sim_value; sim_items = cur_items.copy()
                            existing = next((x for x in sim_items if x["name"] == sim_selected), None)
                            if existing: existing["value"] += sim_value
                            else: sim_items.append({"name": sim_selected, "value": sim_value})
                        else:
                            existing = next((x for x in cur_items if x["name"] == sim_selected), None)
                            if not existing: st.error(f"포트폴리오에 {sim_selected} 종목이 없습니다."); st.stop()
                            sell_val = min(sim_value, existing["value"]); new_total = cur_total - sell_val
                        ...
                        bc2.metric(f"{sim_action} 후 총액", f"{new_total:,.0f}원", f"{new_total - cur_total:+,.0f}원")
                        ...
                        with bc_left:
                            st.markdown("**현재 비중**")
                            fig_cur = go.Figure(go.Pie(labels=[x["name"] for x in cur_items], values=[x["value"] for x in cur_items],
                                hole=0.4, textinfo="label+percent", textfont=dict(size=11), marker=dict(line=dict(color="white", width=2))))
                            fig_cur.update_layout(height=280, margin=dict(l=0,r=0,t=10,b=0), paper_bgcolor="white", showlegend=False)
                            st.plotly_chart(fig_cur, use_container_width=True, key="sim_pie_cur")
                        with bc_right:
                            st.markdown(f"**{sim_action} 후 비중**")
                            fig_sim = go.Figure(go.Pie(labels=[x["name"] for x in sim_items], values=[x["value"] for x in sim_items],
                                hole=0.4, textinfo="label+percent", textfont=dict(size=11), marker=dict(line=dict(color="white", width=2))))
                            fig_sim.update_layout(height=280, margin=dict(l=0,r=0,t=10,b=0), paper_bgcolor="white", showlegend=False)
        ...
        with sim_tab2:
            ...
            with bep_col2:
                ...
                bep_fee = st.number_input("매도 수수료율 (%)", min_value=0.0, value=0.015, step=0.001, format="%.3f", key="bep_fee")

            invested = bep_buy * bep_qty; cur_val = bep_cur * bep_qty
            loss = cur_val - invested; loss_pct = loss / invested * 100
            fee_rate = bep_fee / 100; bep_price = bep_buy / (1 - fee_rate)
            need_pct = (bep_price - bep_cur) / bep_cur * 100

            st.markdown("---")
            r1, r2, r3, r4 = st.columns(4)
            r1.metric("투자 원금", f"{invested:,.0f}원"); r2.metric("현재 평가", f"{cur_val:,.0f}원")
            r3.metric("평가 손익", f"{loss:+,.0f}원", f"{loss_pct:+.2f}%"); r4.metric("손익분기 주가", f"{bep_price:,.0f}원")
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

            if need_pct > 0:
                st.markdown(f"<div style='background:#fff0f0;border-left:4px solid #e03131;border-radius:8px;padding:16px 20px;font-size:14px;color:#333;line-height:1.8;'>본전을 찾으려면 현재가 대비 <b style='color:#e03131;font-size:17px;'>+{need_pct:.2f}%</b> 상승이 필요합니다.<br>현재 손실률 <b>{loss_pct:.2f}%</b>를 회복하려면 <b>{need_pct:.2f}%</b> 더 올라야 합니다. (수수료 {bep_fee:.3f}% 포함)</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='background:#f0fff4;border-left:4px solid #2f9e44;border-radius:8px;padding:16px 20px;font-size:14px;color:#333;line-height:1.8;'>현재 수익 중입니다. 수익률 <b style='color:#2f9e44;font-size:17px;'>{loss_pct:+.2f}%</b></div>", unsafe_allow_html=True)
            ...
            for pct in [5, 10, 15, 20, 30, 50]:
                ...
                scenarios.append({"상승률": f"+{pct}%", "목표 주가": f"{target:,.0f}원",
                                   "예상 손익": f"{profit:+,.0f}원", "손익률": f"{profit/invested*100:+.2f}%",
    ...
    CURRENCIES = {
        "원 (KRW)": "KRW", "달러 (USD)": "USD", "엔 (JPY)": "JPY",
        "유로 (EUR)": "EUR", "위안 (CNY)": "CNY", "캐나다 달러 (CAD)": "CAD",
        "파운드 (GBP)": "GBP", "호주 달러 (AUD)": "AUD",
        "홍콩 달러 (HKD)": "HKD", "싱가포르 달러 (SGD)": "SGD",
    }
    CURRENCY_SYMBOLS = {"KRW": "₩", "USD": "$", "JPY": "¥", "EUR": "€", "CNY": "¥",
                        "CAD": "CA$", "GBP": "£", "AUD": "A$", "HKD": "HK$", "SGD": "S$"}

    def get_rate(from_cur, to_cur):
        if from_cur == to_cur: return 1.0
        try:
            ticker = f"{from_cur}{to_cur}=X"
            t = yf.Ticker(ticker); hist = t.history(period="1d")
            if not hist.empty: return hist["Close"].iloc[-1]
            ticker2 = f"{to_cur}{from_cur}=X"
            t2 = yf.Ticker(ticker2); hist2 = t2.history(period="1d")
            if not hist2.empty: return 1 / hist2["Close"].iloc[-1]
        except: pass
        return None

    if "fx_data" not in st.session_state:
        with st.spinner("환율 불러오는 중..."):
            fx_pairs = {"USD/KRW": "USDKRW=X", "JPY/KRW": "JPYKRW=X",
    ...
    with c2: from_label = st.selectbox("환전 전 통화", list(CURRENCIES.keys()), index=1)
    with c3: to_label = st.selectbox("환전 후 통화", list(CURRENCIES.keys()), index=0)

    from_cur = CURRENCIES[from_label]; to_cur = CURRENCIES[to_label]
    from_sym = CURRENCY_SYMBOLS[from_cur]; to_sym = CURRENCY_SYMBOLS[to_cur]
    from_name = from_label.split(" ")[0]; to_name = to_label.split(" ")[0]

    rate = get_rate(from_cur, to_cur)
    if rate:
        result = amount * rate
        result_str = f"{result:,.0f}" if to_cur in ("KRW", "JPY") else f"{result:,.2f}"
        amount_str = f"{amount:,.0f}" if from_cur == "KRW" else f"{amount:,.2f}"
        st.markdown(f"""
        <div style="background:#f0f7ff; border-radius:12px; padding:24px 32px; margin-top:16px; text-align:center;">
            <span style="font-size:28px; font-weight:600; color:#1a1a2e;">{from_sym}{amount_str} {from_name}</span>
            <span style="font-size:24px; color:#888888; margin:0 16px;">→</span>
            <span style="font-size:28px; font-weight:600; color:#1a3a6e;">{to_sym}{result_str} {to_name}</span>
            <div style="margin-top:10px; font-size:13px; color:#aaaaaa;">1 {from_cur} = {rate:,.4f} {to_cur}</div>
        </div>
        """, unsafe_allow_html=True)
    else:
...
규칙: 날짜는 'M월 D일 (요일) HH:MM' 형식, KST 기준. 대괄호 각주 절대 금지.
...
""")
    ...
    for item in items:
        ...
        for line in lines:
            if line.startswith("날짜:"): date = line.replace("날짜:", "").strip()
            elif line.startswith("지표:"): indicator = line.replace("지표:", "").strip()
            elif line.startswith("국가:"): country = line.replace("국가:", "").strip()
            elif line.startswith("중요도:"): importance = line.replace("중요도:", "").strip()
            elif line.startswith("설명:"): desc = line.replace("설명:", "").strip()
            elif line.startswith("예상치:"): forecast = line.replace("예상치:", "").strip()
        if indicator:
    ...
    st.markdown(
        ...
        "보수적 분석 기준 적용 — 펀더멘털·컨센서스 다수 의견·리스크 동시 검토."
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    def _ai_stock(today):
        ...
        res = _re.sub(r'\[\d+\]', '', res); res = _re.sub(r'\*\*?(.*?)\*\*?', r'\1', res)
        return res
    ...
    def _ai_consensus(today):
        prompt = (
            f"오늘은 {today}이야. 최근 1주일 내 국내 주요 증권사가 발간한 리포트 중 "
            f"목표주가 상향·하향 종목을 각각 3~5개씩 정리해줘.\n\n"
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["추천 종목", "주목 섹터", "위험 종목", "주요 공시", "증권사 컨센서스"])

    with tab1:
        _, col_btn = st.columns([5, 1])
        if col_btn.button("새로고침", key="btn_stock"): _ai_stock.clear(); st.rerun()
        with st.spinner("추천 종목 분석 중..."): stock_res = _ai_stock(today_str)
        for item in parse_items(stock_res):
            f = parse_fields(item, ["종목명","투자의견","목표주가","손절기준","추천근거","리스크","투자기간","확신도"])
            if not f.get("종목명"): continue
            확신 = f.get("확신도","중"); 불가 = "하" in 확신 or "불가" in 확신
            border = "#ef4444" if 불가 else ("#22c55e" if "상" in 확신 else "#6366f1")
            badge = "🚫 추천 불가" if 불가 else ("⭐⭐⭐ 확신 높음" if "상" in 확신 else "⭐⭐ 확신 보통")
            body = (f"<div style='margin-bottom:10px;'><span style='background:{border};color:white;font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;margin-right:8px;'>{badge}</span>"
                    f"<span style='font-size:12px;color:#666;'>투자기간: {f.get('투자기간','-')} | 목표주가: {f.get('목표주가','-')} | 손절기준: {f.get('손절기준','-')}</span></div>"
                    f"<b>추천 근거</b><br>{f.get('추천근거','-')}<br><br><b style='color:#ef4444;'>⚠️ 리스크</b><br>{f.get('리스크','-')}")
            render_card(f"{f.get('종목명','-')}  ({f.get('투자의견','-')})", body, border_color=border)

    with tab2:
        _, col_btn = st.columns([5, 1])
        if col_btn.button("새로고침", key="btn_sector"): _ai_sector.clear(); st.rerun()
        with st.spinner("섹터 분석 중..."): sector_res = _ai_sector(today_str)
        ...
        for i, item in enumerate(parse_items(sector_res)[:3]):
            ...
            body = (f"{f.get('선정이유','-')}<br><br><b style='color:#ef4444;'>⚠️ 리스크</b><br>{f.get('리스크','-')}<br><br>"
                    f"<b>대표 종목</b><br>• {f.get('대표종목1','-')}<br>• {f.get('대표종목2','-')}")
            render_card(f"#{i+1} {f.get('섹터명','-')}", body, border_color=colors[i % 3])

    with tab3:
        st.markdown("<div style='background:#fff0f0;border:1px solid #fca5a5;border-radius:6px;padding:10px 16px;font-size:12px;color:#991b1b;margin-bottom:18px;'>🚨 악재 공시·실적 쇼크·이상 매도 신호 포착 종목. 보유 중이라면 반드시 확인하세요.</div>", unsafe_allow_html=True)
        _, col_btn = st.columns([5, 1])
        if col_btn.button("새로고침", key="btn_warning"): _ai_warning.clear(); st.rerun()
        with st.spinner("위험 신호 스캔 중..."): warning_res = _ai_warning(today_str)
        for item in parse_items(warning_res):
            f = parse_fields(item, ["종목명","위험유형","위험내용","주가영향","대응방안"])
            if not f.get("종목명"): continue
            body = (f"<span style='background:#ef4444;color:white;font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;display:inline-block;margin-bottom:10px;'>{f.get('위험유형','-')}</span><br>"
                    f"<b>위험 내용</b><br>{f.get('위험내용','-')}<br><br><b>주가 영향</b><br>{f.get('주가영향','-')}<br><br>"
                    f"<b style='color:#22c55e;'>💡 대응 방안</b><br>{f.get('대응방안','-')}")
            render_card(f.get("종목명","-"), body, border_color="#ef4444", bg="#fff8f8")

    with tab4:
        _, col_btn = st.columns([5, 1])
        if col_btn.button("새로고침", key="btn_dart"): _ai_dart.clear(); st.rerun()
        with st.spinner("주요 공시 분석 중..."): dart_res = _ai_dart(today_str)
        for item in parse_items(dart_res):
            ...
            방향 = f.get("주가방향","-"); 방향색 = "#22c55e" if "상승" in 방향 else "#ef4444" if "하락" in 방향 else "#6b7280"
            body = (f"<span style='background:{border};color:white;font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;margin-right:6px;'>영향도 {영향도}</span>"
                    f"<span style='background:{방향색};color:white;font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;'>{방향}</span><br><br>"
                    f"<b>공시 유형:</b> {f.get('공시유형','-')}<br><br>{f.get('공시내용','-')}")
            render_card(f.get("종목명","-"), body, border_color=border)

    with tab5:
        _, col_btn = st.columns([5, 1])
        if col_btn.button("새로고침", key="btn_consensus"): _ai_consensus.clear(); st.rerun()
        with st.spinner("증권사 리포트 수집 중..."): consensus_res = _ai_consensus(today_str)
        items = parse_items(consensus_res)
        상향 = [x for x in items if "상향" in x[:60]]; 하향 = [x for x in items if "하향" in x[:60]]
        ...
        for col, group, color, label in [
            ...
            with col:
                st.markdown(f"<div style='font-size:15px;font-weight:700;color:{color};margin-bottom:12px;'>{label}</div>", unsafe_allow_html=True)
                for item in group:
                    f = parse_fields(item, ["종목명","증권사","기존목표가","신규목표가","변경이유"])
                    if not f.get("종목명"): continue
                    body = (f"<b>{f.get('증권사','-')}</b>  |  {f.get('기존목표가','-')} → "
                            f"<b style='color:{color};'>{f.get('신규목표가','-')}</b><br><br>{f.get('변경이유','-')}")
